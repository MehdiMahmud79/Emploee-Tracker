...
const viewRoles = (db, func) => {
  db.query(
    'SELECT  role.title as "job title", role.id as "role id", department.name as "department", role.salary FROM role JOIN department ON department_id = department.id;',
...
const viewEmployees = (db, func) => {
  db.query(
    "SELECT employee_id, first_name, last_name, role.title AS title, role.salary AS salary, manager_id FROM employee JOIN role ON employee.role_id = role.id ORDER BY employee_id;",
    (err, res) => {
      console.log(`\n`);
      console.log(
