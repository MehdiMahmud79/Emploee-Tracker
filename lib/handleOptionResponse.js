...
const printToScreen=(text)=> console.log(colors.brightYellow.bold.bgGreen(`    ${text}   \n`));
...
const viewDepartments = (db, callBackFun) => {
  ...
  FROM 
    ...
    console.log(`\n`);
    console.log(`\n`);
    console.log(colors.brightWhite.bold.bgRed("           Available Departments          \n"));
    console.table(res);
...
const viewRoles = (db, callBackFun) => {
  db.query(`
    ...
    (err, res) => {
      console.log(`\n`);
      console.log(`\n`);
      console.log(colors.brightWhite.bold.bgRed(`                                  Available Roles                               \n`));
      console.table(res);
...
const viewEmployees = (db, callBackFun) => {
  db.query(
    ...
    (err, res) => {
      console.log(`\n`);
      console.log(colors.brightWhite.bold.bgRed(`                                   Current Employees                                                                        \n`
      ));
      console.table(res);
      console.log(`\n`);
...
currentRoles= async(db)=> {
  ...
  const availableRoles = data.map(role => { return {name: role.title, value: role.id} });
...
ManagerList= async(db)=> {
  ...
  const managers = data.map(employee => { return {name: `${employee.first_name}  ${employee.last_name}`, value: employee.employee_id} });
  return managers;
...
const addEmployee = async (db, callBackFun) => {
  ...
  db.query(`INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES ("${employee.firstName}", "${employee.lastName}", ${employee.role}, ${employee.manager});`,(err, res) => {
    if (err) {
      console.error(err)
    } else {
...
currentEmploees= async(db)=> {
  const [data] = await db.promise()
      .query("SELECT employee_id, first_name, last_name FROM employee;");
  const emploees = data.map(employee => { return {name: `${employee.employee_id}: ${employee.first_name} ${employee.last_name}`, value: employee.employee_id} });
  return emploees;
}
...
const updateEmployee = async (db, callBackFun) => {
  ...
  db.query(`UPDATE employee SET role_id = ${employee.Newrole}, manager_id = ${employee.manager} WHERE employee_id = ${employee.employeeId};`,(err, res) => {
    if (err) {
      console.error(err)
    } else {
